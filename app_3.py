...
rl = col4.slider("ρl (%)", 0.8, 2.5, 1.2, step=0.01)
rt = col5.slider("ρt (%)", 0.1, 0.3, 0.15, step=0.01)
t  = col6.slider("CFRP thickness (mm)", 0.0, 2.0, 1.0, step=0.1)

st.markdown("### Material Properties")
col7, col8 = st.columns(2)
fc = col7.slider("Concrete fc (MPa)", 20, 40, 30, step=5)
fy = col8.slider("Steel fy (MPa)", 420, 690, 520, step=10)

col9, col10 = st.columns(2)
ft = col9.slider("CFRP ft (MPa)", 1500, 3140, 2000, step=100)
fb = col10.slider("Bond strength fb (MPa)", 0, 15, 8, step=1)

st.markdown("### Blast Load")
col11, col12 = st.columns(2)
Mass  = col11.slider("Explosive weight W (kg)", 5, 630, 50, step=5.0)
Scale = col12.slider("Scaled distance Z (m/kg¹ᐟ³)", 128, 565, 300, step=5.0)
