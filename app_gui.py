...
rt = col5.slider("Transverse rebar ratio, $\rho_t$ (%)", 0.1, 0.3, 0.15, step=0.01)
t  = col6.slider("CFRP thickness, $\rt_CFRP$ (mm)", 0.0, 2.0, 1.0, step=0.1)

st.markdown("### Material Properties")
col7, col8 = st.columns(2)
fc = col7.slider("Concrete compressive strength, $\rf_c$ (MPa)", 20, 40, 30, step=5)
fy = col8.slider("Steel yield strength, $\rf_y$ (MPa)", 420, 690, 520, step=10)

col9, col10 = st.columns(2)
ft = col9.slider("CFRP tensile strength, $\rf_t$ (MPa)", 1500, 3140, 2000, step=100)
fb = col10.slider("Bond strength, $\rf_b$ (MPa)", 0, 15, 8, step=1)

st.markdown("### Blast Load")
col11, col12 = st.columns(2)
Mass  = col11.slider("Explosive weight, $\rM$ (kg)", 5.0, 630.0, 50.0, step=5.0)
Scale = col12.slider("Scaled distance, $\rZ$ (m/kg¹ᐟ³)", 128.0, 565.0, 300.0, step=5.0)
